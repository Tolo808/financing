...
- The Supabase CLI is installed locally (`admin/node_modules/.bin/supabase`, via the `@supabase/cli-windows-x64` npm package) and `supabase init` has run (`supabase/config.toml`). `supabase link` still needs a Personal Access Token (`supabase login` is an interactive browser flow) — optional, not required for the app itself.
